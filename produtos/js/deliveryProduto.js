$(document).ready(function () {
	...
	$(document).on('click', '.adicionarProduto', function (e) {
		...
		// Override global finalizar just in case
		window.finalizar = function () { return false; };

		// Gather Product Info
		let nomeProduto = $("#detalhesProduto .info1 h3").text().trim();
		let detalheProduto = $("#detalhesProduto .info1 .detalhe").text().trim();
		...
		let cartItem = {
			id: Date.now(),
			name: nomeProduto,
			details: detalheProduto,
			price: finalPrice / qtde, // Unit price approximation
			quantity: qtde,
			observation: obs,
			options: []
		};

		// Populate options for display
		$(".opcoes").each(function () {
			let qtyInput = $(this).find('input.qtdeOpcao');
			let qty = parseInt(qtyInput.val());
			if (qty > 0) {
				let optionName = $(this).find('.nome b').text().trim();
				cartItem.options.push(`${qty}x ${optionName}`);
			}
		});
		...
		// Assuming we are in /produtos/subdir/file.html or /produtos/file.html
		// The checkout is at /checkout.html (root)
		// If we are in /produtos/nonattofamilia.html, we need ../checkout.html
